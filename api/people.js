...
//
// This returns personal data (name, email, organization, title, country) and is
// therefore token-gated the same way /api/support-contacts is: the caller must
// present ?token= equal to BENCH_ADMIN_TOKEN or RUN_TOKEN, the tokens already
// set in Vercel. Without a valid token it returns 401 and no data at all.
//
// It covers every stream in one place, so there is one list to look at:
//   guide-register       field guide registrations (which edition)
...
export default async function handler(req){
  ...
  for (let i = 0; i < rows.length; i++){
    ...
    let detail = '';
    if (src === 'guide-register')      detail = EDITION[String(p.edition || '')] || 'Field guide';
