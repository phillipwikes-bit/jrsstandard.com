...
export async function buildManifest(opts) {
  ...
  // INTEGRITY IS NOT AUTHENTICITY, and this is said here because this is where an
  // implementer reads. manifest_hash proves that THIS manifest has not changed
  // since it was hashed. It does NOT prove who produced it: there is no signing
  // key and no authentication mechanism anywhere in this estate. Anyone can
  // regenerate a modified manifest AND its hash, so detection works only against
  // a copy someone else holds or a hash recorded elsewhere.
  //
  // Added 2026-09-18 (M-10). The distinction was stated in the continuity index
  // and in the claims register as C-16, and was stated NOWHERE beside the code
  // that creates the field. A boundary recorded only in governance documents is
  // not available to the person integrating against this function.
  manifest.integrity = {
