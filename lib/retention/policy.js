...
export const ENGINE_REVIEW_RETENTION = {
  ...
  // DAYS, NOT CALENDAR MONTHS, and the change is deliberate.
  //
  // CORRECTED 2026-09-16 (same day, BD-10 implementation defect, found by the
  // Round F verification pass). This field read `months: 3` and the public
  // disclosure drafted under BD-12 reads "kept for 90 days and then removed".
  // Three calendar months is NOT 90 days: measured back from the 16th it is 92
  // days in January, June and September, 91 in December and 89 in May. On the
  // date the disclosure was written the real window was 92 days, so the page
  // promised removal at 90 and the code kept the text for two days longer.
  // A privacy disclosure that UNDERSTATES retention is wrong in the direction
  // that matters, and check_disclosed_retention_matches_the_policy was papering
  // over it with `days = months * 30`, an assumption written into a guard to
  // make two numbers agree.
  //
  // The calendar-month reasoning below cutoffISO is sound for the 24-month
  // telemetry rule and does not transfer here. Over 24 months a day count
  // drifts across leap years; over 90 days it is a fixed, auditable interval,
  // and the CALENDAR MONTH is the unit that moves. The published commitment is
  // a number a reader can check, so the code now uses that number.
  //
  // BD-10's decision is unchanged: field-level expiry, row survives. Only the
  // unit is corrected.
  days: 90,
...
export function selectExpiringFields(rows, now) {
  const cutoffMs = Date.parse(cutoffDaysISO(now, ENGINE_REVIEW_RETENTION.days));
  const affected = [];
  const untouched = [];
  for (const r of rows || []) {
  ...
  return {
    cutoff: cutoffDaysISO(now, ENGINE_REVIEW_RETENTION.days),
    affected, untouched,
...
// Cutoff by a fixed number of days, for BD-10. A day count is the right unit
// for a short window: it is exactly the number the public disclosure states, so
// a reader can check the commitment, and unlike a calendar month it does not
// change length between February and July. UTC throughout, so the boundary does
// not move with the server's local zone.
export function cutoffDaysISO(now, days) {
  const d = now instanceof Date ? now.getTime() : Date.parse(now);
  return new Date(d - days * 86400000).toISOString();
}
