<!-- PRESERVED VERBATIM 2026-09-14 under CLAUDE.md Rule 10 (PRESERVE HISTORY).

This is the complete repository operating instruction set as it stood before the
JRS Enterprise Asset Orchestrator became the root CLAUDE.md. Nothing was
deleted. The drift-critical invariants (SSOT values, security constraints, the
storage-key registry, design tokens, prose constraints and the deployment
facts) are ALSO carried inline in CLAUDE.md Section 36, because CLAUDE.md is
loaded automatically each session and this annex is not. The long-form
narrative, the forensic record of the Cloudflare skip-token investigation, and
the per-page function reference live here.

OLD FINDING -> NEW EVIDENCE -> CORRECTED STATUS -> EXPLANATION applies to this
file: it is superseded as the ROOT instruction set, and retained in full as the
operational reference it always was.
-->
