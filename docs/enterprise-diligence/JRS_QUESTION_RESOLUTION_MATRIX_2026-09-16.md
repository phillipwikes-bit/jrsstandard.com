...
## 2026-09-18 — B-001 state correction and dependency survivability

| ID | Question | Status |
|---|---|---|
| **B-001** | Vercel credential rotation | **OWNER-CONFIRMED / EXTERNAL ACTION COMPLETED, 2026-09-18.** Corrected from my "PENDING EXTERNAL VERIFICATION", which demanded repository-side proof of an action performed in an external control plane. **No such proof can exist**, so that status made a completed action permanently open. An owner attestation **is** the evidence class this blocker admits; refusing the only admissible evidence is the mirror of upgrading evidence. **Not deployment authorization** |
| **X-12** | The 33 executed contributor consents exist only as Supabase rows | **OPEN — CRITICAL survivability.** E-001–E-004 cite "Live DB, `pilot_contacts source=contributor-confirm`". The Master Register calls these the estate's **strongest rights evidence**. Searching the repository for `contributor-confirm` returns five documents and **every one is an analysis describing the rows, not the rows**. If the project were lost, the estate keeps secondary descriptions of its primary evidence and loses the primary evidence. **Platform backup status NOT ESTABLISHED** and not inferred. **Exporting the rows into the repository is REJECTED** — they carry names, emails and organisations, and a permanent widely-cloned copy of personal data is a worse privacy position than the risk it cures. A controlled export held **outside** the repository, or verified platform backups, is the right shape |
| **X-13** | GitHub and the domain registrar were absent from the dependency register | **CLOSED — both added.** The provider holding the **entire estate** was not listed among its dependencies. GitHub is **MODERATE** (git is distributed; a clone is a real backup in a way a query result is not). The registrar is **MATERIAL** — no stored evidence is lost, but **reachability** is, and every published article and citation resolves through it. Its ownership is separately recorded as UNKNOWN in chain-of-title, so it is both a dependency and an open rights question |
| **Dependency survivability** | If a provider disappeared tomorrow, what evidence is unrecoverable? | **ANSWERED for seven providers.** Classified by **evidence loss**, not by inconvenience: Supabase **CRITICAL**, registrar and Anthropic **MATERIAL**, GitHub **MODERATE**, Vercel / Formspree / Google **LOW**. A provider that is tedious to replace but holds nothing unique is not critical; one that is easy to replace but holds the only copy is |

## 2026-09-18 — misuse control

| ID | Question | Status |
|---|---|---|
| **Misuse control** | Ten modes classified MISUSE → HARM → CONTROL → WARNING → DETECTION | **BUILT.** `.jrs/registries/MISUSE_REGISTER.json`. Five controls anchored to checkable facts in code and verified by guard; four mutations fail. **Seven CONTROLLED, one PARTIALLY (M-5, blocked on X-9), one DETECTABLE-ONLY-AGAINST-A-PRIOR-COPY (M-7), one UNCONTROLLED (M-4)** |
| **X-14** | **M-4 — JRS used to justify discriminatory treatment** | **OPEN — UNCONTROLLED, and it is the largest gap in the register.** JRS evaluates whether a record **explains itself**. A record can be perfectly reconstructable and describe a discriminatory decision, and the engine would route it **"ready"**. **That is not a defect in the evaluation — it is a boundary of what the evaluation means — and nothing currently states that boundary to a user.** A search of the public estate returns one investigator guide and one article source, neither a control on *use*. **Proposed:** one sentence of scope language on the engine surfaces and in the manifest specification — *a reconstructable record is not a defensible decision; JRS evaluates the record, not the decision.* Documentation, within delegated authority, **QUEUED rather than written into a deployable page this cycle.** A guard fails if M-4 is upgraded without a control appearing |
| **M-10** | The integrity-is-not-authenticity boundary was recorded only in governance documents | **CLOSED.** It was stated in the continuity index and as claim C-16, and stated **nowhere beside the code that creates the field**. **A boundary recorded only in governance documents is not available to the person integrating against the function.** Now in `lib/manifest/build.js`, and guarded |
| **M-6 / M-7** | Manifest modification and warning-stripping | **DETECTABLE, WITH A NAMED LIMIT.** The hash detects modification — but **a hash is not a signature**: anyone can regenerate a modified manifest *and* its hash, so detection works only against a copy someone else holds. `warnings` is deliberately **not** schema-required, because a manifest with nothing to warn about legitimately has none — so an absent array is valid and indistinguishable from one that never existed |

## 2026-09-18 — five owner factual confirmations

| ID | Question | Status |
|---|---|---|
| **S-1** | Was de-identification review performed on the 54 records? | **CLOSED — OWNER FACTUAL CONFIRMATION (E-031).** Stacyann Young and Tanvi Pokhriyal entered them through the submission link; the owner **personally reviewed** them for de-identification before use. **Attestation, not row-level evidence** — no per-row artifact exists and none was manufactured. **S-9 falls with it**: it was conditional on the answer being "review not performed" |
| **S-6 / T-6** | Did anyone receive the superseded figures? | **CLOSED — OWNER FACTUAL CONFIRMATION (E-032).** Not provided to or seen by anyone; they stayed within his controlled research work. No independent proof of every historical access event exists and none is claimed. **No notice obligation arises from a distribution that did not happen** |
| **Section 2.1 / V-AI-08** | Whose contribution, and does it await assignment? | **CORRECTED — OWNER IDENTIFICATION (E-033).** V-AI-08 is **Gabriela Cortez**, the owner's mentee at the **MCCR Intake Unit**. The Section 2.1 material is an approach from **her MCCR intake work**, **not a formal contribution to the JRS study**. **HER PANEL PARTICIPATION IS UNAFFECTED AND REMAINS VERIFIED** — Arm A, 24 reads, COMPLETE, named on the AIE title page. **Checked before correcting; a looser correction would have deleted a verified participation record.** **CT-2 closed as framed** |
| **X-15** | Does referencing an approach that originated in state-agency employment carry any third-party interest? | **CLOSED 2026-09-18 — RAISED IN ERROR (E-036).** Two faults: its premise was the superseded reading of E-033 — the approach arose from the **owner's own** MCCR work and experience, discussed with a mentee — and **I created a counsel question from a theoretical possibility with no affirmative evidence.** **Eight propositions searched, ZERO hits**: assignment involving Cortez; documented Section 2.1 contribution; written agreement naming her; MCCR or Maryland claim; employment agreement or policy covering the material; commissioned-work arrangement; communication asserting ownership; any other affirmative evidence. **Removed from the counsel queue and the rights dependency chain.** Closure establishes **no legal conclusion of any kind** — only that the evidence does not support keeping it open. **Her panel participation is a separate fact and is preserved** |
| **CT-1 / Ubayet** | Commercial and successor-transfer scope | **FACTUAL LIMB CLOSED — OWNER ACCOUNT (E-034).** He approved the uses discussed and stated no restriction against commercial or licensing use. **Three things kept apart: approval of the uses discussed (attested); absence of a stated restriction (attested); an express commercial or licensing GRANT (NOT ESTABLISHED).** **Absence of a prohibition is not a grant.** E-021 stands unchanged. **The legal limb stays with counsel under B-004** |
| **Domain registrar** | Who holds `jrsstandard.com`? | **CLOSED AS A FACTUAL QUESTION — OWNER CONFIRMATION (E-035).** He personally controls the registrar account; no one else does. **The survivability class stays MATERIAL and does not improve** — the risk was never *who* holds it, but that a lapse or loss of access breaks every published link, and **single-person control is the definition of that exposure, not a mitigation of it** |

## 2026-09-18 — rights-to-product mapping

| ID | Question | Status |
|---|---|---|
| **Rights-to-product** | What rights would a JRS Review Engine Evaluation Licence actually need, and which are established? | **MAPPED — ten right classes.** Extended into the existing Commercial Rights Register; no competing register created. **NOTHING reaches RIGHT AVAILABLE FOR LICENSING**, because none is CONTRACTUALLY GRANTED and none is LEGALLY INTERPRETED. **That is a statement about instruments and construction, not about the quality of the asset** |
| **X-16** | The sharpest commercial condition | **RECORDED.** The validation evidence a licensee would rely on is **jointly held with four co-author relationships, all outside the co-author instrument (recorded NOT USED, 0 of 3)**, the 33 consents are **silent on licensing**, and reviewer consent is **revocable at will**. **A revocable permission cannot support an exclusive perpetual grant.** A transaction condition, not a defect |
| **Guard** | Can the map drift upward under commercial pressure? | **NO — guarded.** `check_no_right_is_offered_beyond_its_evidence` fails if any right is marked AVAILABLE FOR LICENSING or CONTRACTUALLY GRANTED while F-4 stands, if *absence of a prohibition is not a grant* is removed from the governing facts, if *silence is not permission* is deleted, or if the determination is softened. **Four mutations fail** |
