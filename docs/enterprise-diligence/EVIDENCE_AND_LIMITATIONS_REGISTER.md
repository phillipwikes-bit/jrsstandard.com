...
| A-10 | One current public OpenAPI contract describes the deployed endpoint | API | `openapi.json`; `JRS_PUBLIC_API_CONTRACT_RECONCILIATION_2026-09-22.md` | **VERIFIED correction** | OpenAPI 3.1.0, version `0.1.0-validation`; response conditions remain nested under `result`; current Manifest output is expressly not emitted | **Prior two-contract conflict resolved 2026-09-22** | Owner | public | Former `/openapi-review-engine.json` path permanently redirects to `/openapi.json`; prior conflict remains documented in the reconciliation record |
