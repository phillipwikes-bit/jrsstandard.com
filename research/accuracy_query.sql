...
-- ============================================================
-- VARIANT (pre-registered clean read): only reviewers with >=18 graded reads.
-- This drops in-progress reviewers like RR-108 (9/24) from the comparison.
-- Run this too and compare the Arm B rows to the version above.
-- ============================================================
WITH key(rec, truth) AS (VALUES
  ('R01','G'),('R02','U'),('R03','U'),('R04','G'),('R05','U'),('R06','G'),
  ('R07','U'),('R08','G'),('R09','U'),('R10','G'),('R11','U'),('R12','G'),
...
eligible AS (
  SELECT reviewer_code FROM scored WHERE pred IS NOT NULL
  GROUP BY reviewer_code HAVING count(*) >= 18
)
SELECT
  CASE WHEN s.reviewer_code LIKE 'V-AI-%' THEN 'Arm A detection (expert panel)'
       WHEN s.batch = 'armB-B1' THEN 'Arm B B1 (JRS)'
       WHEN s.batch = 'armB-B2' THEN 'Arm B B2 (baseline)'
       ELSE 'other' END AS arm,
  count(DISTINCT s.reviewer_code) AS reviewers,
  count(*) FILTER (WHERE s.pred IS NOT NULL) AS graded_reads,
  round(100.0 * avg((s.pred = s.truth)::int) FILTER (WHERE s.pred IS NOT NULL), 1) AS accuracy_pct
FROM scored s JOIN eligible e ON e.reviewer_code = s.reviewer_code
