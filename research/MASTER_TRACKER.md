...
- **Attribution limitation:** `enrollSubmit()` tags `audience='panel'` only when `?src=panel`, else `'public'`; `source` is hardcoded `'training.html'`. Arbitrary `?src` values (e.g. `linkedin`) are NOT captured, so a LinkedIn campaign's enrollments land under `public`. A ~2-line change would record any `?src` tag for per-campaign attribution. Not built (not requested as built).
