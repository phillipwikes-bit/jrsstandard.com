...
| 7 | Supabase token ROTATED 2026-07-13 (old token now returns 401, good). A NEW token is needed only for DDL (applying SQL / new tables); Panel reviewer registration is already token-free via `/api/register` (service-role), and reads work via the anon key. **Clean DDL path found:** set a fresh Supabase personal access token as env var `SUPABASE_ACCESS_TOKEN` in the web environment settings, and `scripts/supabase-apply.py` (run automatically by the SessionStart hook, or manually) applies `supabase-ALL.sql` via the Management API: no SQL editor, no token in chat. | Provision `SUPABASE_ACCESS_TOKEN` in environment settings (then the enroll table + any pending DDL apply automatically), or use the SQL editor once | You |
