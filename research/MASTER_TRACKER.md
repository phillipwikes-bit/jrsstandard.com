...
**REFERRAL OUTREACH DRAFTED (2026-07-15):** three emails to Peter's referrals in `research/Referral_Outreach_Emails.md`, following his suggestion to contact them. FELTG/Deborah Hopkins (`&src=feltg`), Gary Gilbert (`&src=ggilbert`), LRP (`&src=lrp`). Each offers the free Investigator Field Guides + the new training (invite link `?access=k7m2p9x4t1c8&src=<tag>`) and carries the DRR paragraph. Integrity: Peter invoked only as having SUGGESTED the introduction (not as endorser; he disclaimed endorsement on record); no proven-effectiveness claim; em-dash in the supplied DRR paragraph converted to a comma per house rule. Blind spots flagged in chat (competitor-framing, Peter's name discipline, LRP contact unknown).
