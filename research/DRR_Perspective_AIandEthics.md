...
For Gabi this was not an abstract ideal but a daily practice: she often translated intake records into Spanish so that a language barrier never cost someone clear, honest information about a decision that affected them. She also held that collaboration across cultures and backgrounds is what makes a decision resilient enough to survive challenge, a conviction that later led me to stress-test this work through an international panel of reviewers spanning many countries, languages, and domains.
...
Named instruments show the pattern. The NIST AI Risk Management Framework organizes governance around functions for governing, mapping, measuring, and managing risk across the AI lifecycle. ISO/IEC 42001 specifies an AI management system at the organizational level. The OECD AI Principles set out high-level values for trustworthy AI. Each is valuable, and each operates on the system, the organization, or the process. None asks the record-level question at issue here: whether the specific artifact a workflow produces contains, on its own terms, the basis a later reviewer would need to reconstruct and defend the decision. DRR names that residual, output-level question, and a control for it is complementary to these frameworks rather than a substitute for them.

DRR is also distinct from three neighboring ideas it is easily confused with. It is not documentation quality in general: a record can be fluent, thorough, and well formatted and still fail, because reconstructability turns on whether the decision's basis is present, not on how polished the prose is. It is not model explainability: explainability asks why a system produced an output and looks inside the model, whereas DRR asks whether the record itself carries the basis, whoever or whatever wrote it. And it is not records management: retention, storage, findability, and chain of custody concern a record's lifecycle, while DRR concerns the evidentiary self-sufficiency of a record's reasoning. DRR is a content property of the artifact, and it is the property that accountability actually depends on.

This is why the control has to sit at the level of the record and has to be workflow-independent. By workflow-independent I mean independent of the drafting method and technology, not independent of an organization's process: such a control is meant to slot into existing review, not to replace it. It should not matter whether a record was written by a person, by a person assisted by AI, or by an automated pipeline. The governance question is the same: can a later, independent reviewer reconstruct and defend the decision from the record alone? Documentation reviewability deserves to be treated as a first-class governance control, complementary to model-centric ones, not folded into them.
